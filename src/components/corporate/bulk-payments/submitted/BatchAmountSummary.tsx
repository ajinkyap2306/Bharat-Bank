...
export const BatchAmountSummary: React.FC<BatchAmountSummaryProps> = ({ data }) => (
  <section className="mx-4 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-5 text-center">
    <p className="text-[28px] font-bold text-slate-900 dark:text-white tracking-tight">
      {formatPaymentCurrency(data.totalAmount)}
    </p>
    <p className="text-[14px] font-medium text-slate-500 dark:text-slate-400 mt-1">{data.paymentCount} Payments</p>
    <div className="mt-4 pt-4 border-t border-slate-200 dark:border-slate-800">
      <p className="text-[12px] text-slate-500 dark:text-slate-400">Total Debit</p>
      <p className="text-[22px] font-bold text-slate-900 dark:text-white mt-0.5">
    ...
    {data.status === 'completed' || data.status === 'partially_completed' ? (
      <div className="flex justify-center gap-6 mt-3 text-[13px]">
        <span className="text-emerald-600 dark:text-emerald-400 font-semibold">{data.successfulCount} Successful</span>
        {data.failedCount > 0 && (
      ...
      <p className="text-[13px] text-emerald-600 dark:text-emerald-400 font-semibold mt-3">{data.validCount} Valid</p>
    ) : null}
  </section>
);
