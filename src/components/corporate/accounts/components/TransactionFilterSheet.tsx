...
const Chip: React.FC<{ active: boolean; label: string; onClick: () => void }> = ({
  ...
  <button
    ...
    className={`px-3 py-1.5 rounded-full text-xs font-bold ${
      active ? 'bg-congress-blue-700 text-white' : 'bg-slate-100 dark:bg-slate-800 text-slate-500 dark:text-slate-400'
    }`}
  >
    {label}
  </button>
  ...
  return (
    <BottomSheet isOpen={isOpen} onClose={onClose} title="Filter Transactions" subtitle="Refine your transaction list">
      <div className="space-y-4 pb-2">
        <div>
          <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Type</p>
        ...
        <div>
          <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Status</p>
          <div className="flex flex-wrap gap-1.5">
        ...
        <div>
          <p className="text-[10px] font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider mb-2">Date</p>
          <div className="flex flex-wrap gap-1.5">
            {([
              ['today', 'Today'],
              ['7d', '7 Days'],
        ...
        <div className="flex gap-2 pt-2">
          <button type="button" onClick={reset} className="flex-1 py-3 rounded-2xl border border-slate-200 dark:border-slate-700 font-bold text-sm text-slate-500 dark:text-slate-400">
            Reset
          </button>
          <button type="button" onClick={apply} className="flex-1 py-3 rounded-2xl bg-congress-blue-700 text-white font-bold text-sm">
            Apply Filters
          </button>
        </div>
      </div>
