...
export const StatementSearch: React.FC<StatementSearchProps> = ({ query, onChange, onFilter }) => (
  <div className="px-4 flex gap-2">
    ...
    <button
      type="button"
      onClick={onFilter}
      className="shrink-0 px-3 rounded-xl border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-[13px] font-semibold text-slate-500 dark:text-slate-400 min-h-11"
    >
      Filter
    </button>
  </div>
