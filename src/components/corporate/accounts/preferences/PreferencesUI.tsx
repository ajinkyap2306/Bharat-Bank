...
export const PreferencesToggle: React.FC<{
  ...
  <div className="flex items-center justify-between gap-3 px-4 py-3.5 border-b border-slate-200 dark:border-slate-800/80 dark:border-slate-800 last:border-0 min-h-14">
    <div className="min-w-0 flex-1">
      <label htmlFor={id} className="text-[14px] font-medium text-slate-900 dark:text-white block">
        {label}
      </label>
      {description && (
        <p id={`${id}-desc`} className="text-[12px] text-slate-500 dark:text-slate-400 mt-0.5">
          {description}
        </p>
      )}
    </div>
    <button
      ...
      className={`w-12 h-7 flex items-center rounded-full p-0.5 transition-colors shrink-0 motion-reduce:transition-none ${
        checked ? 'bg-congress-blue-700' : 'bg-slate-200 dark:bg-slate-700'
      } ${disabled ? 'opacity-50' : ''}`}
    >
      <span
