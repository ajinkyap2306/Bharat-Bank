...
export const TransactionSkeleton: React.FC = () => (
  <div className="mx-4 space-y-3">
    {[1, 2, 3, 4].map((i) => (
      <div key={i} className="flex gap-3 p-4 rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E7EC] animate-pulse">
...
export const TransactionEmptyState: React.FC<TransactionEmptyStateProps> = ({
  ...
  <div className="mx-4 p-8 rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E7EC] text-center">
    <div className="w-14 h-14 rounded-2xl bg-[#0B5CAB]/10 flex items-center justify-center mx-auto mb-3">
      {hasFilters ? (
        <Receipt className="w-7 h-7 text-[#0B5CAB]" aria-hidden />
      ) : (
        <ArrowLeftRight className="w-7 h-7 text-[#0B5CAB]" aria-hidden />
      )}
    </div>
    <p className="text-base font-semibold text-[#111827] dark:text-white">No transactions found</p>
    ...
    {hasFilters && (
      <button
        ...
        className="mt-4 text-[13px] font-semibold text-[#0B5CAB] min-h-11 px-4"
      >
        Clear Filters
      </button>
    )}
