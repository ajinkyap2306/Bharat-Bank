  ...
  <section className="px-4" aria-label="Corporate Relationship">
    ...
    <div className="rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-4 shadow-sm space-y-3">
      <div className="flex items-start justify-between gap-3">
        <span className="text-[13px] text-slate-500 dark:text-slate-400">Company</span>
        <span className="text-[13px] font-medium text-slate-900 dark:text-white text-right">
          {account.companyName}
        </span>
      </div>
      <div className="flex items-start justify-between gap-3">
        <span className="text-[13px] text-slate-500 dark:text-slate-400">Relationship</span>
        <span className="text-[13px] font-medium text-slate-900 dark:text-white text-right">
          {relationship}
        </span>
      </div>
      {account.relationshipManager && (
        <div className="flex items-start justify-between gap-3">
          <span className="text-[13px] text-slate-500 dark:text-slate-400">Relationship Manager</span>
          <span className="text-[13px] font-medium text-slate-900 dark:text-white text-right">
            {account.relationshipManager}
          </span>
        </div>
