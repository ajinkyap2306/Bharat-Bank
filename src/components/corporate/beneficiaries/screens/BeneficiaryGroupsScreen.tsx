    ...
    <div className="space-y-2 pt-2">
      {groups.map((g) => {
        ...
        return (
          ...
          >
            <span className="text-sm font-bold text-[#111827] dark:text-white">{g.name}</span>
    ...
    <div className="-mx-3 bg-[#F7F9FC] dark:bg-slate-950 min-h-full pb-6">
      ...
      {members.length === 0 ? (
        <BenCard className="p-6 text-center">
          <p className="text-sm text-[#667085]">No beneficiaries in this group yet.</p>
        </BenCard>
      ) : (
        <div className="space-y-2 pt-2">
          {members.map((b) => (
            ...
            >
              <div>
                <p className="text-sm font-bold text-[#111827] dark:text-white">{b.name}</p>
                <p className="text-xs text-[#667085]">{b.bankName} • {b.maskedAccount}</p>
              </div>
