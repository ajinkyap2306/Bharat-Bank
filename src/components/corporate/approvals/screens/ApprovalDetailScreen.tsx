...
export const ApprovalDetailScreen: React.FC<ApprovalDetailScreenProps> = ({
  ...
  return (
    <div className="-mx-3 bg-[#F7F9FC] dark:bg-slate-950 min-h-full pb-32">
      ...
      <div className="px-3 space-y-3">
        ...
        {request.expiresAt && (
          <ApprovalCard className="p-3 border-amber-200 bg-amber-50/50">
            ...
            <p className="text-[10px] text-[#667085]">{request.expiresAt}</p>
          </ApprovalCard>
        )}

        <ApprovalCard className="p-4">
          <p className="text-xs font-bold text-[#667085] uppercase mb-2">Request Summary</p>
        ...
        {(request.amount !== undefined || request.beneficiaryName) && (
          <ApprovalCard className="p-4">
            <p className="text-xs font-bold text-[#667085] uppercase mb-2">
        ...
        <ApprovalCard className="p-4">
          ...
          <p className="text-sm font-bold text-[#111827] dark:text-white">{request.createdBy}</p>
          <p className="text-xs text-[#667085]">Role: {request.createdByRole}</p>
          <p className="text-xs text-[#667085] mt-1">{request.createdAt}</p>
        </ApprovalCard>

        {request.amount !== undefined && request.amount > 0 && (
          <ApprovalCard className="p-4">
            <p className="text-xs font-bold text-[#667085] uppercase mb-2">Your Approval Limit</p>
            <p className="text-lg font-mono font-bold text-[#111827] dark:text-white">{formatApprovalCurrency(APPROVER_LIMIT)}</p>
            <p className="text-xs text-[#667085] mt-2">Payment Amount</p>
            ...
            {overLimit ? (
              ...
              <p className="text-xs font-bold text-[#16A34A] mt-2 flex items-center gap-1">
        ...
        <ApprovalCard className="p-4">
          <p className="text-xs font-bold text-[#667085] uppercase mb-2">Approval Policy</p>
          <p className="text-xs text-[#667085] mb-3">{request.currentApprovals} of {request.requiredApprovals} approvals completed</p>
          {request.approvalLevels.map((level) => (
            <div key={level.level} className="flex items-center gap-3 py-2 border-b border-slate-100 dark:border-slate-800 last:border-0">
              <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${
                level.status === 'approved' ? 'bg-[#16A34A] text-white' :
                level.status === 'pending' ? 'bg-[#0B5CAB] text-white' : 'bg-slate-200 text-slate-400'
              }`}>
                {level.status === 'approved' ? '✓' : level.status === 'pending' ? '●' : '○'}
              </div>
              <div className="flex-1">
                <p className="text-xs font-bold text-[#111827] dark:text-white">Level {level.level}</p>
                <p className="text-[10px] text-[#667085]">{level.role}</p>
                {level.name && <p className="text-[10px] text-[#667085]">{level.name}</p>}
              </div>
              {level.date && <span className="text-[10px] text-[#667085]">{level.date}</span>}
            </div>
          ))}
        </ApprovalCard>
        ...
        <ApprovalCard className="p-4">
          <div className="flex items-center justify-between mb-3">
            <p className="text-xs font-bold text-[#667085] uppercase">Approval History</p>
            <button type="button" onClick={onAudit} className="text-[10px] font-bold text-[#0B5CAB]">View Audit</button>
          </div>
          {request.timeline.map((event) => (
            <div key={event.id} className="flex gap-3 py-2">
              <div className={`w-5 h-5 rounded-full flex items-center justify-center text-[10px] shrink-0 mt-0.5 ${
                event.status === 'completed' ? 'bg-[#16A34A] text-white' :
                event.status === 'pending' ? 'bg-[#0B5CAB] text-white' : 'bg-slate-200 text-slate-400'
              }`}>
                {event.status === 'completed' ? '✓' : event.status === 'pending' ? '●' : '○'}
              </div>
              <div>
                <p className="text-xs font-bold text-[#111827] dark:text-white">{event.label}</p>
                <p className="text-[10px] text-[#667085]">{event.user}</p>
                <p className="text-[10px] text-[#667085]">{event.date} • {event.time}</p>
                {event.comments && <p className="text-[10px] text-[#667085] italic mt-0.5">{event.comments}</p>}
              </div>
            </div>
          ))}
        </ApprovalCard>
        ...
        {request.notes && (
          <ApprovalCard className="p-4">
            <p className="text-xs font-bold text-[#667085] uppercase mb-1">Notes</p>
            <p className="text-sm text-[#111827] dark:text-white">{request.notes}</p>
          </ApprovalCard>
        )}
      ...
      {request.status === 'pending' && canApprove && !isSelfRequest && !overLimit && (
        <div className="fixed bottom-20 left-0 right-0 z-20 px-3">
          <div className="max-w-lg mx-auto">
            <button type="button" onClick={onRequestChanges} className="w-full py-3 rounded-xl border border-[#0B5CAB] text-[#0B5CAB] font-bold text-sm bg-white dark:bg-slate-900 min-h-11">
              Request Changes
            </button>
          </div>
        </div>
    ...
    <div className="px-3 space-y-2">
      {request.timeline.map((event) => (
        <ApprovalCard key={event.id} className="p-4">
          <p className="text-[10px] text-[#667085]">{event.date} • {event.time}</p>
          <p className="text-sm font-bold text-[#111827] dark:text-white mt-1">{event.label}</p>
          <p className="text-xs text-[#667085]">{event.user}</p>
          {event.comments && <p className="text-xs text-[#667085] mt-2 italic">{event.comments}</p>}
        </ApprovalCard>
      ))}
    </div>
  </div>
