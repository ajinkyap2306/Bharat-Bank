...
function getHeroContent(data: VendorPaymentSubmissionData): {
  ...
  switch (data.status) {
    ...
    case 'approved':
      return {
        icon: <Loader2 className="w-12 h-12 text-[#0B5CAB] animate-spin motion-reduce:animate-none" aria-hidden />,
...
export const PaymentStatusState: React.FC<PaymentStatusStateProps> = ({ data }) => {
  ...
  >
    ...
    <h2 className="text-[17px] font-semibold text-[#111827] dark:text-white">{hero.title}</h2>
    <p className="text-[13px] text-[#667085] mt-1.5 max-w-[280px] mx-auto">{hero.description}</p>
    ...
    {data.status === 'rejected' && (
      <div className="mt-4 pt-4 border-t border-[#E4E7EC] dark:border-slate-800 text-left space-y-2">
        {data.rejectedBy && (
