...
export const NextActionCard: React.FC<NextActionCardProps> = ({
  ...
  if (!data.approvalRequired) {
    return (
      ...
      >
        <h2 id="next-action-heading" className="text-[14px] font-semibold text-[#111827] dark:text-white">
          Next Action
        </h2>
        <p className="text-[13px] text-[#111827] dark:text-white mt-2 font-medium">
  ...
  return (
    ...
    >
      ...
      <p className="text-[12px] text-[#667085] mt-1">{data.nextActionDescription}</p>
      <button
        type="button"
        onClick={onViewApprovalStatus}
        className="mt-3 w-full flex items-center justify-center gap-2 py-3 rounded-xl bg-[#0B5CAB] text-white text-[14px] font-semibold min-h-11 active:scale-[0.98] transition-transform focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B5CAB] focus-visible:ring-offset-2"
      >
