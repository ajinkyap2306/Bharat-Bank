  ...
  return (
    <section className="px-4" aria-labelledby="payment-purpose-label">
      <button
        ...
        onClick={onOpen}
        className="w-full rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E7EC] dark:border-slate-800 p-4 text-left min-h-14 active:bg-slate-50 dark:active:bg-slate-800/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B5CAB] focus-visible:ring-offset-2"
      >
        <div className="flex items-center justify-between gap-3">
          <div>
            ...
            <p className="text-[14px] text-[#111827] dark:text-white mt-1">{label}</p>
          </div>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center">
      ...
      <div className="relative w-full max-w-lg bg-white dark:bg-slate-900 rounded-t-3xl p-5 pb-safe max-h-[70vh] overflow-y-auto">
        <h3 className="text-[16px] font-semibold text-[#111827] dark:text-white mb-3">
          Payment Purpose
        </h3>
        <div className="space-y-2">
          {PAYMENT_PURPOSE_OPTIONS.map((opt) => (
            ...
            >
              <p className="text-[14px] font-semibold text-[#111827] dark:text-white">{opt.label}</p>
            </button>
          ))}
        </div>
      </div>
