...
export const ProcessingEstimate: React.FC<ProcessingEstimateProps> = ({ method }) => (
  <section className="px-4" aria-label="Estimated processing">
    <div className="rounded-2xl bg-[#0B5CAB]/5 border border-[#0B5CAB]/15 p-4 flex gap-3">
      <div className="w-10 h-10 rounded-xl bg-white dark:bg-slate-900 flex items-center justify-center shrink-0">
        <Clock className="w-5 h-5 text-[#0B5CAB]" aria-hidden />
      </div>
      <div>
        <p className="text-[13px] font-semibold text-[#111827] dark:text-white">
          Estimated Processing
        </p>
        <p className="text-[14px] font-bold text-[#0B5CAB] mt-0.5">{method}</p>
        <p className="text-[12px] text-[#667085] mt-1">{getProcessingEstimate(method)}</p>
        <p className="text-[11px] text-[#667085] mt-2 italic">Estimate only. Processing times may vary.</p>
      </div>
    </div>
  </section>
);
