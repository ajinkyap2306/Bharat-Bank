...
export const PaymentSubmissionError: React.FC<PaymentSubmissionErrorProps> = ({
  ...
  <section className="px-4" role="alert" aria-labelledby="submission-error-title">
    <div className="rounded-2xl bg-white dark:bg-slate-900 border border-[#DC2626]/30 p-5 text-center shadow-sm">
      ...
      <h2 id="submission-error-title" className="text-[16px] font-semibold text-slate-900 dark:text-white">
      ...
      <button
        type="button"
        onClick={onRetry}
        className="mt-4 inline-flex items-center gap-2 px-6 py-3 rounded-2xl bg-congress-blue-700 text-white text-[14px] font-semibold min-h-11"
      >
      ...
      <button
        type="button"
        onClick={onReview}
        className="mt-2 block w-full py-3 text-[14px] font-semibold text-slate-500 dark:text-slate-400 min-h-11"
      >
        Review Payment
      </button>
    </div>
