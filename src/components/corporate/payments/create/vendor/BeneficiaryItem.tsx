...
export const BeneficiaryItem: React.FC<BeneficiaryItemProps> = ({
  ...
  if (variant === 'favorite') {
    return (
      <button
        ...
        disabled={!selectable}
        aria-label={srLabel}
        className="shrink-0 w-[200px] rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E7EC] dark:border-slate-800 p-4 text-left shadow-sm min-h-[120px] active:bg-slate-50 dark:active:bg-slate-800/40 disabled:opacity-60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#0B5CAB]"
      >
        <div className="flex items-start justify-between gap-2">
          <div
            className="w-10 h-10 rounded-xl bg-[#0B5CAB]/10 flex items-center justify-center text-[#0B5CAB] text-[13px] font-bold"
            aria-hidden
          >
            {initials}
          </div>
        ...
        <p className="text-[14px] font-semibold text-[#111827] dark:text-white mt-3 line-clamp-2">
          {beneficiary.name}
        </p>
        <p className="text-[12px] text-[#667085] tabular-nums mt-1">{beneficiary.maskedAccountNumber}</p>
        <p className="text-[12px] text-[#667085] mt-0.5">{beneficiary.bankName}</p>
        <div className="mt-2">
  ...
  return (
    <div
      className={`rounded-2xl bg-white dark:bg-slate-900 border border-[#E4E7EC] dark:border-slate-800 overflow-hidden ${
    ...
    >
      <button
        ...
        disabled={!selectable}
        aria-label={srLabel}
        className="w-full flex items-center gap-3 p-4 text-left min-h-[76px] active:bg-slate-50 dark:active:bg-slate-800/40 disabled:cursor-not-allowed focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0B5CAB]"
      >
        <div
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="text-[13px] text-[#667085] tabular-nums mt-0.5">
        ...
        <div className="flex items-center gap-1 shrink-0">
          ...
          {selectable && <ChevronRight className="w-5 h-5 text-[#667085]" aria-hidden />}
        </div>
      </button>

      {!selectable && statusMessage && (
        <div className="px-4 pb-4 pt-0 border-t border-[#E4E7EC]/80 dark:border-slate-800">
          <p className="text-[12px] text-[#667085] mt-3 leading-relaxed">{statusMessage}</p>
