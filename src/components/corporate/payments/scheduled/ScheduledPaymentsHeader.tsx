...
export const ScheduledPaymentsHeader: React.FC<ScheduledPaymentsHeaderProps> = ({
  ...
  <header className="sticky top-0 z-20 bg-[#F7F9FC]/95 dark:bg-slate-950/95 backdrop-blur-md px-4 pt-3 pb-2 safe-top">
    <div className="flex items-center gap-1 max-w-[430px] mx-auto">
      ...
      <h1 className="text-[17px] font-bold text-[#111827] dark:text-white flex-1 truncate">{title}</h1>
      {onCreate && (
        <button
          type="button"
          onClick={onCreate}
          className="w-9 h-9 flex items-center justify-center rounded-xl bg-[#0B5CAB] text-white shrink-0"
