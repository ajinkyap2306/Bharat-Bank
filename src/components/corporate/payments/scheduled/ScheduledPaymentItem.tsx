...
export const ScheduledPaymentItem: React.FC<ScheduledPaymentItemProps> = ({
  ...
  <button
    ...
    className="w-full flex items-start gap-3 p-4 min-h-[80px] active:bg-slate-50 dark:active:bg-slate-800/40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#0B5CAB]"
  >
    <span className="w-10 h-10 rounded-xl bg-blue-50 dark:bg-blue-950/40 flex items-center justify-center shrink-0 mt-0.5">
      <Calendar className="w-5 h-5 text-[#0B5CAB]" aria-hidden />
    </span>
    <div className="flex-1 min-w-0 text-left">
      <p className="text-[14px] font-semibold text-[#111827] dark:text-white truncate">{item.beneficiaryName}</p>
      <p className="text-[15px] font-bold text-[#111827] dark:text-white mt-0.5 tabular-nums">
    ...
    <ChevronRight className="w-4 h-4 text-[#667085] shrink-0 mt-3" aria-hidden />
  </button>
);
