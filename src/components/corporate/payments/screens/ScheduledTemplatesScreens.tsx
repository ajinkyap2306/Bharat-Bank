            ...
            <PayCard key={s.id} className="p-4">
              <div className="flex justify-between items-start">
                <div>
                  ...
                  <p className="text-xs text-[#667085]">{s.beneficiaryName}</p>
                  <p className="text-sm font-mono font-bold mt-1">{formatPaymentCurrency(s.amount)}</p>
                  <p className="text-[10px] text-[#667085]">Due {s.scheduledDate} • {s.frequency}</p>
                </div>
      ...
      <div className="space-y-2">
        {templates.map((t) => (
          <PayCard key={t.id} className="p-4">
            ...
            <p className="text-xs text-[#667085]">{t.beneficiaryName}</p>
            <p className="text-sm font-mono font-bold">{formatPaymentCurrency(t.amount)}</p>
            <p className="text-[10px] text-[#667085]">{t.purpose} • {t.reference}</p>
            <div className="flex gap-2 mt-3">
              <button
                type="button"
                onClick={() =>
                ...
                className="flex-1 py-2 rounded-xl bg-[#0B5CAB] text-white text-xs font-bold min-h-11"
              >
                Use Template
              </button>
          ...
          <PayCard key={d.id} className="p-4 mb-2">
            <div className="flex justify-between">
              <div>
                ...
                <p className="text-[10px] text-[#667085]">{d.updatedAt}</p>
              </div>
              <PaymentStatusBadge status="Draft" />
            </div>
            <div className="flex gap-2 mt-3">
              <button type="button" onClick={() => onContinue(d.form)} className="flex-1 py-2 rounded-xl bg-[#0B5CAB] text-white text-xs font-bold min-h-11">Continue</button>
