...
export const ReviewRow: React.FC<{ label: string; value: string; onEdit?: () => void }> = ({
  ...
  <div className="flex items-start justify-between gap-3 py-2.5 border-b border-slate-100 dark:border-slate-800 last:border-0">
    <div className="min-w-0">
      ...
      <p className="text-sm font-medium text-[#111827] dark:text-white wrap-break-word">{value}</p>
    </div>
    {onEdit && (
      <button type="button" onClick={onEdit} className="text-xs font-bold text-[#0B5CAB] shrink-0">
        Edit
      </button>
    )}
  </div>
    ...
    <div className={`max-w-lg mx-auto flex gap-2 ${secondaryLabel ? '' : ''}`}>
      {secondaryLabel && onSecondary && (
        <button
          type="button"
          onClick={onSecondary}
          className="flex-1 py-3.5 rounded-2xl border border-slate-200 dark:border-slate-700 font-bold text-sm text-[#667085]"
        >
          {secondaryLabel}
        </button>
      )}
      <button
        ...
        onClick={onClick}
        disabled={disabled}
        className="flex-1 py-3.5 rounded-2xl bg-[#0B5CAB] text-white font-bold text-sm disabled:opacity-50 min-h-11"
      >
        {label}
      </button>
    </div>
...
export const MenuPayRow: React.FC<{
  ...
  >
    <div className="w-10 h-10 rounded-xl bg-[#0B5CAB]/10 text-[#0B5CAB] flex items-center justify-center shrink-0">
      {icon}
    </div>
