...
export const VerificationStatus: React.FC<VerificationStatusProps> = ({
    ...
    return (
      ...
      >
        <motion.div
          ...
          className="w-16 h-16 rounded-2xl bg-congress-blue-700/10 flex items-center justify-center mb-4 motion-reduce:animate-none"
        >
          <ShieldCheck className="w-8 h-8 text-congress-blue-700" aria-hidden />
        </motion.div>
            ...
            <div className="flex flex-wrap gap-3 mt-3">
              {onTryAgain && (
                <button
                  type="button"
                  onClick={onTryAgain}
                  className="text-[13px] font-semibold text-congress-blue-700 min-h-11 px-1"
                >
                  Try Again
                </button>
              )}
              {onUseOtp && (
                <button
                  type="button"
                  onClick={onUseOtp}
                  className="text-[13px] font-semibold text-slate-500 min-h-11 px-1"
                >
                  Use OTP Instead
                </button>
              )}
  ...
  return (
    ...
    >
      ...
      <p className="text-[14px] text-slate-500 mt-1.5">{message}</p>
    </motion.div>
  );
};
